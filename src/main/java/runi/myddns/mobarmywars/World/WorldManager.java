package runi.myddns.mobarmywars.World;
...
public class WorldManager {
        ...
        if (!worldExists("world_mobarmylobby")) {
            Bukkit.getConsoleSender().sendMessage(
                    ConsoleColor.BLOOD_ORANGE +
                            "       Lobby-Welt wird aus ZIP entpackt" +
                            ConsoleColor.RESET
            );
                ...
                Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    ...
                    Bukkit.getScheduler().runTaskLater(plugin, () -> {
                        ...
                        for (Player player : Bukkit.getOnlinePlayers()) {
                            ...
                            player.sendMessage(ChatColor.DARK_RED + "  diese können nur über " +
                                    ChatColor.GOLD + "Reset Spielfortschritt ");
    ...
    private void checkTeamWorlds() {
        ...
        if (!rotWorld || !rotNether) {
            Bukkit.getConsoleSender().sendMessage(
                    ConsoleColor.DARK_RED +
                            "     TEAM-Welten von ROT werden erzeugt!" + ConsoleColor.RESET);
            if (!rotWorld)
            ...
            Bukkit.getConsoleSender().sendMessage(
                    ConsoleColor.LIME + "           Teamwelt - " +
                            ConsoleColor.RED + "ROT " +
                            ConsoleColor.LIME + "vorhanden!" +
                            ConsoleColor.RESET);
        }

        if (!blauWorld || !blauNether) {
            Bukkit.getConsoleSender().sendMessage(
                    ConsoleColor.DARK_AQUA +
                            "     TEAM-Welten von BLAU werden erzeugt!" + ConsoleColor.RESET);
            if (!blauWorld)
